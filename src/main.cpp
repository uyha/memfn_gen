...
int main(int argc, char **argv) {
    ...
    options.add_options()
            ("f,format", "the format to be printed out following fmt syntax, "
                         ...
                         "and prefixed with `mN_` with masked functions while N is from 0 to the number of masks supplied",
             value(format))
            ("c,config", "const volatile ref noexcept config of the member function",
             value(config)->default_value("3373"))
            ("m,mask", "the masks for transforming the member function", value(masks))
            ("return_type", "the return type of the function type",
             value(variable_formatter.rtn_type)->default_value("R"))
            ("class_type", "the class type of the function type",
             value(variable_formatter.cls_type)->default_value("T"))
            ("name", "name of the variable", value(variable_formatter.var_name)->default_value("fn"))
            ("arg_type", "the argument type of the function type",
             value(variable_formatter.arg_type)->default_value("Args..."))
            ...
            ("volatile_prefix", "volatile prefix of the alias",
             value(alias_formatter.volatile_prefix)->default_value("volatile_"))
            ("cv_prefix", "cv prefix of the alias", value(alias_formatter.cv_prefix)->default_value("cv_"))
            ("noexcept_prefix", "noexcept prefix of the alias",
             value(alias_formatter.noexcept_prefix)->default_value("noexcept_"))
            ("empty_prefix", "prefix of the alias for no reference qualifier", value(alias_formatter.empty_prefix))
            ("lvalue_prefix", "lvalue prefix of the alias",
             value(alias_formatter.lvalue_prefix)->default_value("lvalue_"))
            ("rvalue_prefix", "rvalue prefix of the alias",
             value(alias_formatter.rvalue_prefix)->default_value("rvalue_"))
            ("h,help", "Print usage");
    // clang-format on
